...
.people{display:flex;gap:5px;flex-wrap:wrap;margin:2px 0}
.pchip{display:inline-flex;align-items:center;gap:4px;font-size:11.5px;font-weight:600;padding:3px 9px 3px 7px;border-radius:999px;border:1.5px solid;line-height:1.35}
.pchip .g{font-weight:800;font-size:12px}
.pchip.s-yes{background:var(--yes-bg);color:var(--yes);border-color:#cfe3cd}
.pchip.s-maybe{background:var(--maybe-bg);color:var(--maybe);border-color:#ecdcb4}
.pchip.s-no{background:var(--no-bg);color:var(--no);border-color:#eccabf}
.pchip.me{box-shadow:0 0 0 1.5px rgba(60,44,24,.14)}
...
let _psig="";
let cat="all", stf=new Set(), tagf=new Set(), personf="all";
let ITEMS=[],CATS=[],byId={};
let shared={items:{},cats:{}}; // shared catalog from Firebase (/picks/_catalog)
function rebuild(){
  const im={}; st.custom.items.forEach(i=>im[i.id]=i); Object.values(shared.items||{}).forEach(i=>{if(i&&i.id)im[i.id]=i;});
  const cm={}; st.custom.cats.forEach(c=>cm[c.key]=c); Object.values(shared.cats||{}).forEach(c=>{if(c&&c.key)cm[c.key]=c;});
  ITEMS=BITEMS.concat(Object.values(im)); CATS=BCATS.concat(Object.values(cm));
...
function submitAdd(){
  ...
  const it={id:'ui'+Date.now().toString(36)+Math.random().toString(16).slice(2,5),cat:secKey,type:type,name:name,custom:true,link:type==='product'?gv('a_url'):''};
  if(type==='color'){it.hex=document.getElementById('a_hex').value;const cd=gv('a_code');if(cd)it.code=cd;it.price=null;it.tags=[];}
  else{const p=gv('a_price');it.price=p?Number(p):null;const img=gv('a_img');if(img)it.img=img;const tg=gv('a_tag');it.tags=tg?[tg]:[];}
  st.custom.items.push(it); save();
  if(newCat) fbPushCatalogCat(newCat);
  fbPushCatalogItem(it);
...
function allPeople(){
  ...
  if(me){ const items={}; Object.keys(st.s).forEach(id=>{const s=st.s[id]; if(s&&s.status&&s.status!=='none')items[id]={s:s.status};});
...
function statusOfName(name,id){ for(const p of allPeople()){ if(p.name===name){ const m=p.items[id]; if(m&&m.s&&m.s!=='none') return m.s; } } return 'none'; }
function marksFor(id){ const out=[]; namesList().forEach(name=>{ const s=statusOfName(name,id); if(s!=='none') out.push({name,status:s,mine:me&&name===me.name}); }); return out; }
function renderPeople(){
  els.forEach((c,id)=>{const box=c.querySelector('.people'); if(!box)return; const ms=marksFor(id);
    box.innerHTML=ms.map(m=>`<span class="pchip s-${m.status}${m.mine?' me':''}"><span class="g">${GL[m.status]}</span>${m.name}</span>`).join('');
