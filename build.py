...
.pmark .ext{font-size:11px;color:var(--muted);font-weight:500}
.pmark .ext:before{content:"↳ ";opacity:.6}
...
function markOf(name,id){ for(const p of allPeople()){ if(p.name===name){ const m=p.items[id]; if(m&&m.s&&m.s!=='none') return m; } } return null; }
...
function renderPeople(){
  els.forEach((c,id)=>{const box=c.querySelector('.people'); if(!box)return; const ms=marksFor(id);
    box.innerHTML=ms.map(m=>{
      const showExt=!m.mine && ((m.qty&&m.qty>1)||(m.note&&m.note.trim()));
      const ext=showExt?`<span class="ext">${m.qty>1?'×'+m.qty:''}${m.note&&m.note.trim()?(m.qty>1?' · ':'')+m.note.trim():''}</span>`:'';
      return `<div class="pmark"><span class="pchip s-${m.status}${m.mine?' me':''}"><span class="g">${GL[m.status]}</span>${m.name}</span>${ext}</div>`;
    }).join('');
